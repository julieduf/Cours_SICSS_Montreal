...
```{r}
library(readr)
X2020_06_clean_mturk_data <-
  read_csv("/Volumes/Transcend/SICSS2023/Cours_SICSS_Montreal/Semaine2_Formation_CSS/Jour4.2_Enquetes_ere_digitale/Données/2020_06_clean_mturk_data.csv")
View(X2020_06_clean_mturk_data)
pew_benchmark_question_source_sicss_2020 <-read_csv("/Volumes/Transcend/SICSS2023/Cours_SICSS_Montreal/Semaine2_Formation_CSS/Jour4.2_Enquetes_ere_digitale/Données/pew_benchmark_question_source_sicss_2020.csv")
View(pew_benchmark_question_source_sicss_2020)
```
...
```{r}
#enlever les données socio-démographique 

summary(data) #questions individuelles
class(data$educ)
names(data)

data2 <-select(data, -c(sex, educ,race, age_cat,region))
names(data2)

data2_mean <- data2 %>% 
  summarise_all(mean, na.rm = TRUE) #appliquer la même fonction sur toute la base de données

#pivot_long

data2_mean_pivot <- data2_mean %>%
  pivot_longer(COVIDNEWSSWITCH.1 : ALG_JOBCANDIDATE.1, names_to = "qid", values_to = "mean")

data2_pew_mean <- inner_join(pew, data2_mean_pivot, by ="qid")
...
ggplot(data2_pew_mean) + 
  geom_point(aes(x = mean, y = pew_estimate)) +
  geom_smooth(aes(x = mean, y = pew_estimate, 
                  color = mean), 
              method = lm, se = FALSE,  formula = y ~ x) + 
  labs(title = "Association entre les moyennes estimées et les moyennes calculées",
...
```{r}
#erreur <- data2_pew_mean %>%
  #mutate_all("pew_estimate" - "mean")

erreur <- data2_pew_mean$pew_estimate - data2_pew_mean$mean

data2_pew_mean_erreur <- data2_pew_mean %>%
  mutate(erreur = data2_pew_mean$pew_estimate - data2_pew_mean$mean)

ggplot(data2_pew_mean_erreur, aes(x = erreur, y = qid)) +
  geom_point() + 
   labs(title = "Différence entre les moyennes estimées et les moyennes calcultées pour chaque indicateur",
       x = "erreur",
       y = "qid") +
  theme(legend.position = "bottom", legend.direction = "horizontal")
...
sum_population <- sum(census$POP)

# calculer les poids des groupes

## regrouper les données de population par sexe et par région

population_sexe_region <- census %>%
  group_by(sex, region) %>% 
  summarise(pop_sex_region = sum(POP)) 

## obtenir la somme pour chaque cellule et diviser par la pop totale
population_sexe_region <- population_sexe_region %>%
  mutate(weight_pop = pop_sex_region/sum_population)
...
data_census <- inner_join(data_sex_region, population_sexe_region, by = c("sex", "region"))

##calculer les moyennes selon le sexe et la région
...
data_census_long <- data_census %>% 
  pivot_longer(cols = COVIDNEWSSWITCH.1:ALG_JOBCANDIDATE.1, names_to = "qid", values_to = "mean")



# moyennes pondérées par la somme, regroupement par question

data_mean2 <- data_census_long %>% 
  mutate(mean2 = mean * weight_pop) %>% 
  group_by(qid) %>% 
  summarise(new_mean = sum(mean2))
...
graph_association <- ggplot(data2_pew_mean) + 
  geom_point(aes(x = mean, y = pew_estimate)) +
  geom_smooth(aes(x = mean, y = pew_estimate, 
                  color = mean), 
              method = lm, se = FALSE,  formula = y ~ x) + 
