...
crsc96 <- read_csv("/Volumes/Transcend/SICSS2023/Cours_SICSS_Montreal/Semaine1_Formation_Intensive_R/Jour2_Manipulation/Données/cora-crsc1996-E-1996_F1.csv")
...
Dans toute étude, il est important de résumer l'information contenue dans les variables pour se faire une première idée.
...
getmode <- function(v) {
   uniqv <- unique(v)
   uniqv[which.max(tabulate(match(v, uniqv)))] # pour obtenir le mode
}
...
## Statistiques univariées sur plusieurs variables, solution alternative
...
L'age mode est beaucoup plus bas que la moyenne et la médiane d'âge.
...
Ce fichier comprend les informations sur le poids et la taille de 200 individus ainsi que leur poids et taille autodéclarés. On veut voir de quelle manière les poids auto-déclarés sont fiables. 
